...
class Persistence {
  ...
  async persistNewStateAsync(newDocs) {
    ...
    await this.storage.appendFileAsync(this.filename, toPersist, this.modes.fileMode)
  }
  ...
  async loadDatabaseAsync() {
    ...
    if (this.storage.readFileStream) {
      // Server side
      const fileStream = this.storage.readFileStream(this.filename, this.modes.fileMode)
      treatedData = await this.treatRawStreamAsync(fileStream)
    } else {
      // Browser
      const rawData = await this.storage.readFileAsync(this.filename, this.modes.fileMode)
