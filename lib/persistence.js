...
class Persistence {
   ...
   * @param {Object} [options.storage] Storage module to use, defaults to the best available for the environment
   */
  constructor (options) {
    this.db = options.db
  ...
  async persistCachedDatabaseAsync () {
    ...
    await this.storage.crashSafeWriteFileLinesAsync(this.filename, lines, this.modes)
  ...
  async persistNewStateAsync (newDocs) {
    ...
    await this.storage.appendFileAsync(this.filename, toPersist, { encoding: 'utf8', mode: this.modes.fileMode })
  }
  ...
  async loadDatabaseAsync () {
    ...
    await this.storage.ensureDatafileIntegrityAsync(this.filename, this.modes.fileMode)

    let treatedData
    if (this.storage.readFileStream) {
      // Server side
      const fileStream = this.storage.readFileStream(this.filename, { encoding: 'utf8', mode: this.modes.fileMode })
      treatedData = await this.treatRawStreamAsync(fileStream)
    } else {
      // Browser
      const rawData = await this.storage.readFileAsync(this.filename, { encoding: 'utf8', mode: this.modes.fileMode })
  ...
  async dropDatabaseAsync () {
    ...
    if (!this.db.inMemoryOnly) {
      await this.db.executor.pushAsync(async () => {
        if (await this.storage.existsAsync(this.filename)) await this.storage.unlinkAsync(this.filename)
      }, true)
    }
  }
  ...
  static async ensureParentDirectoryExistsAsync (dir, mode = DEFAULT_DIR_MODE) {
    return this.storage.ensureParentDirectoryExistsAsync(dir, mode)
  }
}
